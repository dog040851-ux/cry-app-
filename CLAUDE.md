...
## TWA (Play스토어)
- .well-known/assetlinks.json 이 앱과 이 사이트를 연결한다. 이게 맞아야 주소창이 사라진다.
- 지문을 추가할 때는 sha256_cert_fingerprints 배열에 항목만 더한다.
  객체를 통째로 복사해 넣지 말 것. 같은 패키지는 한 statement 안에 모은다.
